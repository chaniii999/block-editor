...
**1차 마무리·미해결:** `p_docs/07_마무리_및_알려진_한계.md`
...
6. **상속 선**은 **`specEdgeRouter.js`**가(일반), **포함=상속 부모**는 **선 없이 헤더 🔼** (`nestedSpecChrome`).
7. 그 외 선·하이라이트는 **`MxEdgeBuilder.js`**, **`MxNeighborHighlight.js`** 등.

즉, “선이 왜 이렇게 꺾이지?”는 **spec/MxEdgeBuilder**, “박스가 왜 여기 있지?”는 **ELK·bddLayout·layout.js precompute**입니다.
...
| `media/editor/mxgraph/specEdgeRouter.js` | **상속** 선 직교 라우팅 (N→S) |
| `media/editor/model/nestedSpecChrome.js` | 포함 부모=spec 부모 → 크롬 플래그 |
| `media/editor/layout.js` | 헤더 폭 사전계산 (`precomputeNodeSizes`) |
| `media/editor/mxgraph/MxNeighborHighlight.js` | 선택·연관·연결·nested spec 부모 HL |
| `media/editor/mxgraph/MxFoldManager.js` | 접기 오버레이 |
| `media/editor/mxgraph/edgeOrthogonalPolicy.js` | “선은 직교로” **공통 규칙** (로드 시) |
...
## 7. 마무리·알려진 한계 (2026-05-20)

| 항목 | 설명 |
|------|------|
| **nested spec 크롬** | test-9: 자식이 **직접 포함 부모**에게 spec — 🔼, 선 숨김 |
| **여러 부모 자식 UI** | test-8·다중 `__in__` — **해결 불가(보류)** |

→ **`p_docs/07_마무리_및_알려진_한계.md`**

---

## 8. 정리

- **README의 “표 안 품질”을 선 위주로 맞추려면** → `specEdgeRouter.js` + `MxEdgeBuilder.js` + (로드 시) `edgeOrthogonalPolicy.js`.  
- **박스 위치·간격**을 맞추려면 → `displaySettings.js` + `elkLayout.js` + `bddLayout.js` + `layout.js`.  
